...
public class Heat extends UniversalActor  {
	...
	public Heat(UAN __uan, UAL __ual, Object obj) {
		...
		if (obj instanceof UniversalActor.State || obj==null) {
			  ...
			  if (__ual != null && !__ual.getLocation().equals(ServiceFactory.getTheater().getLocation())) {
			    ...
			    createRemotely(__uan, __ual, "examples.Heat.Heat", sourceRef);
			  }
	...
	public class State extends UniversalActor .State {
		...
		public State(UAN __uan, UAL __ual) {
			super(__uan, __ual);
			addClassName( "examples.Heat.Heat$State" );
		...
		public double[][] doWork(double[][] data, int iterations, int noActors, String nameServer, String theatersFile) {
			...
			for (int k = 0; k<iterations; k++){
				for (int i = 1; i<x-1; i++)for (int j = 1; j<y-1; j++)next[i][j] = 0.25*(data[i-1][j]+data[i][j-1]+data[i+1][j]+data[i][j+1]);
				double[][] temp = next;
				next = data;
				data = temp;
			}
		...
		public void produceFile(double[][] data, String fileName) {
			try {
				...
				out.println(x+" "+y);
				for (int i = 0; i<x; i++){
					for (int j = 0; j<y; j++)out.println(data[i][j]);
				}
				out.flush();
				out.close();
			}
		...
		public void act(String args[]) {
			...
			y += 2;
			{
				...
				// produceFile(token, fileName)
				{
					...
					Message message = new Message( self, self, "produceFile", _arguments, token_2_1, null );
					__messages.add( message );
				}
			}
		}
